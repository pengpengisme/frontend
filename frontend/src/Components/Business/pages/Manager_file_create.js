function Manager_file_create({data}){
    const editClick = (pId) => {
            ...
            {data.map((item)=>{
                ...
                <div className="bag_order">
                    ...
                    <div className="bag_order_info padding">
                        ...
                        <button className='btn_one' key={item.pid} onClick={() => editClick(item.pId)}><div>去建檔</div></button>
                    </div>
                </div>
                );
            })}
...
export default Manager_file_create;